...
flow_item = flow.create_new_flow_batch(
    ...
    context="Generated by Model X",   # Ranking flows only: batch-level text context
    ...
    media_contexts=[...],             # Optional: image/video/audio path(s)/URL(s) per datapoint (each entry a single asset or a list)
